...
| CORS allows `localhost:3002`. Production `cars-api.benwer.es` currently **403s** browser requests whose `Origin` is `https://marketplace.benwer.es` (and `https://admin.cars.benwer.es`). | The marketplace browser client always uses the same-origin `/api` BFF. Server Components and the BFF call `API_ORIGIN` without forwarding `Origin`. The API still needs to allow the marketplace origin for any future direct browser calls. |
